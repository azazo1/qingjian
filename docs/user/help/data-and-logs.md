...
每天向官网 qingjian.app 读取一次版本列表（一个静态文件），有新版本时在输入法菜单（macOS）、任务栏「中 / 英」图标的右键菜单（Windows）与设置的「关于」页提示，不自动下载、不自动安装。
请求不带账号、设备标识或任何输入内容，只有 HTTP 请求本身带的信息（IP 地址、`qingjian/<版本号>` 的 User-Agent）。
版本列表带数字签名，验证不通过的列表一律忽略。在「关于」页关掉「自动检查更新」后不再联网检查。
...
| `user-words.tsv` | 自动造词, 云端选中, 以及菜单「录入词组」记下的用户词 | 选过的词 |
...
| `update.json` | 上次检查更新的时间与查到的新版本 | 否 |
