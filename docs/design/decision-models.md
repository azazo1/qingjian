...
整句候选的顺序换过之后, 光看候选看不出 "这次排序是谁定的, 它做了什么", 所以壳会在被重排过的候选后面
标一个小字: 决策模型是 `AI 76% ↑2`, 字级模型是 `AI ↑2`. 不是词前的图标 (那是云端词的云朵, 说的是候选从哪来,
与排序无关), 而是跟在候选词之后, 译文之前的第一个 annotation 片段, 用强调色.

两栏数字的来源不同:

- **位移 `↑N` / `↓N`** (`ModelHint::shift`): 重排前后名次的变化, 两种打分器都算得出 —— 字级模型一样在改排序.
- **置信度 `76%`** (`ModelHint::confidence`): 只有相对分才算得出. 分就是 `概率 x span`, 所以
  `SentenceScorer::relative_scale()` 报出 span 之后, Core 把 `分 / span` 记下来. 字级模型的整句 log 概率
  不是概率 (按字累加, 长短句不可比), 硬折算成百分比只是假精度, 所以它那一栏不显示.
