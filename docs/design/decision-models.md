...
云端判断要一两秒, 而整句候选的顺序换过之后光看候选看不出"这次排序是谁定的", 所以壳在被重排过的候选后面
标一个 `AI 87%`: 不是词前的图标 (那是云端词的云朵, 说的是候选从哪来, 与排序无关), 而是跟在候选词之后,
译文之前的第一个 annotation 片段, 用强调色.

数字的来源: 相对分的分就是 `概率 x span`, 所以 `SentenceScorer::relative_scale()` 报出 span 之后, Core 在
重排时顺手把 `分 / span` 记成每条候选的置信度 (`Engine::model_confidence`), 壳画候选行时按文本取回.
字级模型报不出这个值 (它的 log 概率只有相对大小, 不是概率), 所以只接本地模型时不会出现徽标.
