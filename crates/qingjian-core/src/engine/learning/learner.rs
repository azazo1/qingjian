...
pub trait Learner: Send {
    ...
    /// 用户在候选窗口里手动调频（调频键 + K / J）：升频按「又选了一次」记，降频按「撤销一次选择」记。
    /// `input` 是当前组句的输入串（候选覆盖的那段拼音，不含分隔符）；返回调整之后的
    /// （全局选择次数, 该输入串下的选择次数）。两个数都已经是 0 时降频什么都不改。
    ///
    /// 与 [`Self::forget`] 一样，这是用户在候选窗口里明确要求的管理操作，私密输入里照做。
    fn adjust_frequency(&mut self, _input: &str, _text: &str, _up: bool) -> (u32, u32) {
        (0, 0)
    }
